...
def sample_M(m, n, t, number_of_holes, x_holes, y_holes):
    result = np.ones((m, n, t))
    random_idx = np.random.multivariate_normal([14, 14], [[5, 0], [0, 5]], number_of_holes).round().astype(int)
    for z in range(m):
        for s in random_idx:
            for i in range(y_holes):
                for j in range(x_holes):
                    if s[0] +j < n and s[1]+i<t:
...
for it in tqdm(range(10000)):
    ...
    M_mb = sample_M(mb_size, DimX,DimY, 5,5,5 )
    H_mb1 = sample_M(mb_size, DimX,DimY, 5,5,5 )
    H_mb = M_mb * H_mb1

    New_X_mb = M_mb * X_mb + (1 - M_mb)   # Missing Data Introduce
    ...
    if it % 100 == 0:
        ...
        Z_mb = sample_Z(5, Dim)
        M_mb = sample_M(5, DimX,DimY, 5,5,5)

        New_X_mb = M_mb * X_mb + (1 - M_mb)

        samples1 = X_mb
        samples5 = M_mb * X_mb + (1 - M_mb)
        ...
        fig = plot(samples)
        plt.savefig('bigger/{}.png'.format(str(i).zfill(3)), bbox_inches='tight')
        i += 1
        plt.close(fig)
